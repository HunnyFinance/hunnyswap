...
// Mint XOXO for LP providers
// Does not support single XOXO pool and tokens with transfer fees!!!!
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime; // Last block time that XOXOs distribution occurs.
    ...
    // xoxo tokens created per second.
    uint256 public xoxoPerSecond;

    // set a max xoxo per second, which can never be higher than 1 per second
    uint256 public constant MaxXoXoPerSecond = 1e18;
    uint256 public constant MaxAllocPoint = 10000; // max 100x
    ...
    // The block time when xoxo mining starts.
    ...
    constructor(
        XoXoToken _xoxo,
        address _devaddr,
        uint256 _xoxoPerSecond,
        uint256 _startTime
    ) {
        xoxo = _xoxo;
        devaddr = _devaddr;
        xoxoPerSecond = _xoxoPerSecond;
    ...
    // Changes xoxo token reward per second, with a cap of max xoxo per second
    // Good practice to update pools without messing up the contract
    function setXoXoPerSecond(uint256 _xoxoPerSecond) external onlyOwner {
        require(_xoxoPerSecond <= MaxXoXoPerSecond, "setXoXoPerSecond: too many xoxos!");

        // This MUST be done or pool rewards will be calculated with new xoxo per second
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken) external onlyOwner {
        ...
        poolInfo.push(
            PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardTime: lastRewardTime, accXoXoPerShare: 0})
        );
    }
    ...
    function pendingXoXo(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 xoxoReward = multiplier.mul(xoxoPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        xoxo.mint(devaddr, xoxoReward.div(10));
    ...
    // Deposit LP tokens to MasterChef for xoxo allocation.
    ...
    function transferXoXoOwnership(address _newOwner) external onlyOwner {
        Ownable(address(xoxo)).transferOwnership(_newOwner);
    }
}
