...
// Dependency file: contracts/gXoXoToken.sol
...
contract gXoXoToken is Ownable, ERC20Votes {
    constructor() ERC20("XOXO Governance Token", "gXOXO") ERC20Permit("gXoXoToken") {}
...
// Root file: contracts/LockingChef.sol
...
// import "contracts/gXoXoToken.sol";

// XOXO locking chef
contract LockingChef is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct UserLocking {
        ...
        uint256 govPower; // equal to gXOO amount
        uint256 releaseTime; // time when user can unlock
    }

    // Info for each user vesting
    struct UserVesting {
        uint256 payout; // total XOXO reward
        uint256 vesting; // vesting period (in seconds)
        uint256 lastTime; // last claim reward time
    }

    XoXoToken public xoxo;
    gXoXoToken public gXoXo;

    // how many XOXO reward per second per XOXO locked
    uint256 public xoxoPerShare;
    uint256 public totalAllocPoint;

    // XOXO balance for payout
    ...
    function estimateReward(uint256 _term, uint256 _amount) public view returns (uint256) {
        ...
        return _amount.mul(xoxoPerShare).mul(term.lockingTerm).mul(term.allocPoint).div(totalAllocPoint).div(1e18);
    }
    ...
    function percentRewardFor(address _locker) public view returns (uint256) {
        ...
        if (vesting > 0) {
            return secsSinceLast.mul(10000).div(vesting);
        } else {
            return 0;
        }
    }
    ...
    function pendingRewardFor(address _locker) external view returns (uint256) {
        ...
        if (percentVested >= 10000) {
            ...
            return payout.mul(percentVested).div(10000);
        }
    }
    ...
    function startLock(uint256 _term, uint256 _amount) external nonReentrant {
        ...
        uint256 govPower = estimateGovPower(_term, _amount);
        gXoXo.mint(msg.sender, govPower);

        user.currentTerm = _term;
        user.principal = _amount;
        user.govPower = govPower;
        user.releaseTime = block.timestamp.add(term.lockingTerm);

        xoxo.transferFrom(msg.sender, address(this), _amount);

        emit Locking(msg.sender, _amount, term.lockingTerm, govPower, payout);
    }
    ...
    function increaseLockAmount(uint256 _amount) external nonReentrant {
        ...
        _updateRewardFor(msg.sender, terms[user.currentTerm].lockingTerm, payout);

        uint256 govPower = estimateGovPower(user.currentTerm, _amount);
        gXoXo.mint(msg.sender, govPower);

        user.principal = user.principal.add(_amount);
        user.govPower = user.govPower.add(govPower);
    ...
    function endLock() external nonReentrant {
        ...
        _safeXoXoTransfer(msg.sender, user.principal);
    ...
    function _claimRewardFor(address _locker) internal {
        ...
        if (percentVested >= 10000) {
            ...
            _safeXoXoTransfer(_locker, info.payout);
        } else {
            // if unfinished
            ...
            _safeXoXoTransfer(_locker, payout);
        }
    }

    // Safe xoxo transfer function, just in case if rounding error causes pool to not have enough XOXOs.
    function _safeXoXoTransfer(address _to, uint256 _amount) internal {
    ...
    function setXoXoPerShare(uint256 _xoxoPerShare) external onlyOwner {
        xoxoPerShare = _xoxoPerShare;
    }

    // deposit XOXO as reward
    function deposit(uint256 _amount) external onlyOwner {
        totalReward = totalReward.add(_amount);
        xoxo.transferFrom(msg.sender, address(this), _amount);
    }

    // withdraw XOXO reward
    function withdraw(uint256 _amount) external onlyOwner {
        totalReward = totalReward.sub(_amount);
        xoxo.transfer(msg.sender, _amount);
    }
